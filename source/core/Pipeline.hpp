...
class SizeComputer;
/** pipeline. one session may contains multiple pipeline, and one pipeline may
...
public:
  ...
  Pipeline(Schedule::PipelineInfo &&info, bool allocInput, bool outputStatic,
           const TuningAttr &tune, const Runtime *rt, const Runtime *cpuRt);
  ~Pipeline();
  class UnitInfo : public OperatorInfo {
  public:
    UnitInfo() = default;
    virtual ~UnitInfo() = default;
    void setUp(const Command &cmd, int index, const Op *originOp,
               int totalIndex);
  };
...
public:
  ...
  */
  ...
  ErrorCode executeCallBack(const TensorCallBackWithInfo &before,
