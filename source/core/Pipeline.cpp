...
static bool _supportQuant(const Op *op, const std::vector<Tensor *> &inputs,
  ...
  case OpType_ConvolutionDepthwise:
    if (op->main_as_Convolution2D() &&
        op->main_as_Convolution2D()->weight() != nullptr) {
      return false;
    } else {
      return true;
    }
  ...
  case OpType_Raster: {
    for (auto input : inputs) {
      if (TensorUtils::getDescribe(input)->quantAttr.get() !=
  ...
  case OpType_ReLU:
    if (TensorUtils::getDescribe(inputs[0])->quantAttr.get() !=
        TensorUtils::getDescribe(outputs[0])->quantAttr.get()) {
      return false;
    }
  ...
  case OpType_Pooling:
    if (op->main_as_Pool() && op->main_as_Pool()->type() == PoolType_MAXPOOL) {
      return true;
    } else if (op->main_as_Pool() &&
               op->main_as_Pool()->type() == PoolType_AVEPOOL) {
      return true;
    } else {
      return false;
    }
...
void Pipeline::UnitInfo::setUp(const Command &command, int index,
                               const Op *originOp, int totalIndex) {
  ...
  if (command.op->type() == OpType_UnaryOp) {
    mContent->type =
        EnumNameUnaryOpOperation(command.op->main_as_UnaryOp()->opType());
  } else if (command.op->type() == OpType_BinaryOp) {
    mContent->type = EnumNameBinaryOpOperation(
        (BinaryOpOperation)(command.op->main_as_BinaryOp()->opType()));
  } else if (command.op->type() == OpType_Reduction) {
    mContent->type = EnumNameReductionType(
        command.op->main_as_ReductionParam()->operation());
  } else {
  ...
  mContent->flops =
      SizeComputer::computeFlops(command.op, command.inputs, command.outputs);
#endif
}

Pipeline::Pipeline(Schedule::PipelineInfo &&info, bool allocInput,
                   bool outputStatic, const TuningAttr &tune, const Runtime *rt,
                   const Runtime *cpuRt)
#ifndef MNN_BUILD_MINI
    : mContext(info.first.cache.second, info.first.cache.first->type(),
               info.first.info.user ? info.first.info.user->precision
...
ErrorCode Pipeline::encode(bool supportDebug, bool permitCodegen) {
  ...
  if (!mInfo.first.needComputeGeometry) {
    for (int i = 0; i < mInfo.second.size(); ++i) {
      ...
      if (cmd->op->type() == OpType_Raster) {
        ...
        if (TensorUtils::getDescribe(info.outputs[0])->regions.empty() &&
            info.inputs.size() > 0 &&
  ...
  if (mIsQuantModel && (mBackend->type() == MNN_FORWARD_CPU ||
                        mBackend->type() == MNN_FORWARD_CPU_EXTENSION ||
                        mBackend->type() == MNN_FORWARD_CUDA ||
                        mBackend->type() == MNN_FORWARD_NN ||
                        mBackend->type() == MNN_FORWARD_OPENCL)) {
    // get propagate map
    using PropagateMap =
        std::map<const MNN::Tensor *, std::set<const MNN::Tensor *>>;
    PropagateMap forwardMap, backwardMap;
    auto insertPropagateMap = [](PropagateMap &propagateMap, const Tensor *s,
                                 const Tensor *t) {
    ...
    std::set<OpType> propagateOpTypes = {
        OpType_Raster,     OpType_ReLU,     OpType_ReLU6,
        OpType_Pooling,    OpType_Interp,   OpType_CropAndResize,
        OpType_ROIPooling, OpType_Gather,   OpType_GatherV2,
        OpType_GatherV2,   OpType_ScatterNd};
    ...
    // propagate scale
    auto propagateScale = [](PropagateMap &propagateMap,
                             std::set<const Tensor *> &start) {
      std::function<bool(const Tensor *)> scalePropagate =
          [&propagateMap, &scalePropagate](const Tensor *t) {
            if (TensorUtils::getDescribe(t)->quantAttr.get() == nullptr) {
              return false;
            }
            ...
            for (auto x : propagateMap[t]) {
              if (TensorUtils::getDescribe(x)->quantAttr !=
                  TensorUtils::getDescribe(t)->quantAttr) {
                TensorUtils::getDescribe(x)->quantAttr =
                    TensorUtils::getDescribe(t)->quantAttr;
                change = true;
              }
    ...
    for (int i = 0; i < 3 && (propagateScale(forwardMap, forwardStart) ||
                              propagateScale(backwardMap, backwardStart));
         i++)
      ;
    ...
    for (auto &info : mInfo.second) {
      ...
      for (auto cmdP : bufferCommand) {
        ...
        auto makeCommand = [&cachedCastTensor, &info](CommandBuffer &cmdBuffer,
                                                      Tensor *input,
                                                      DataType runType) {
          ...
          command->buffer->storage = builder.ReleaseRaw(
              command->buffer->allocated_size, command->buffer->offset);
        ...
        if (OpType_Raster == opType) {
          ...
          for (int i = 0; i < cmd.inputs.size(); i++) {
            if (OpCommonUtils::opNeedContent(cmd.op, i) &&
                inputs[i]->getType() != halide_type_of<int>()) {
              bool needCast = CPUBackend::getDataType(inputs[i]) != runType;
              if (needCast) {
                cmd.inputs[i] =
                    makeCommand(info.executeBuffer, inputs[i], runType);
              }
            }
          }
        }
  ...
  if (supportDebug) {
    ...
    for (auto &info : mInfo.second) {
      ...
      for (auto &cmdP : buffer.command) {
        ...
        static_cast<UnitInfo *>(cmd.info.get())
            ->setUp(cmd, index++, info.op, totalIndex++);
    ...
    for (auto &info : mInfo.second) {
      ...
      for (auto &cmdP : buffer.command) {
        mFlops +=
            SizeComputer::computeFlops(cmdP->op, cmdP->inputs, cmdP->outputs);
      }
    }
  }
#endif
...
void Pipeline::_pushTuningTask(std::vector<Schedule::OpCacheInfo> &&initInfos) {
    ...
    for (int v = 0; v < buffer.command.size(); ++v) {
      ...
      auto copyTensor = [&](std::vector<Tensor *> &tensors) {
        for (int v = 0; v < tensors.size(); ++v) {
          ...
          TensorUtils::getDescribe(newTensor.get())->regions =
              TensorUtils::getDescribe(t)->regions;
  ...
  auto future = std::async(
      std::launch::async,
      [&, this](std::vector<Schedule::OpCacheInfo> &&infos,
        ...
        for (auto &info : infos) {
          ...
          for (auto &iterP : buffer.command) {
            ...
            const_cast<Runtime *>(mRuntime)->onMaskOpReady(
                iter.inputs, iter.outputs, iter.op);

            // If create op failed, we can also mask the op is ready for runtime
            std::shared_ptr<Execution> exe(
                backend->onCreate(iter.inputs, iter.outputs, iter.op));
            if (nullptr == exe) {
              continue;
            }
      ...
      std::ref(const_cast<Runtime *>(mRuntime)->mCancelled));
...
static void _makeCopyOp(std::shared_ptr<BufferStorage> &copyOp) {
  if (copyOp.get() == nullptr) {
    ...
    copyOp->storage =
        builder.ReleaseRaw(copyOp->allocated_size, copyOp->offset);
  }
}
static ErrorCode
_InsertCopy(Schedule::PipelineInfo &mInfo,
            std::map<Tensor *, std::shared_ptr<Tensor>> &mCacheConstTensors,
            std::map<Tensor *, std::shared_ptr<Tensor>> &shapeFixConstCache,
            bool ownInput, bool permitCodegen) {
  ...
  for (auto &info : mInfo.second) {
    ...
    for (auto &iterP : commands) {
      ...
      for (int v = 0; v < iter.inputs.size(); ++v) {
        ...
        if (WrapExecution::needWrap(t, curBackend)) {
          do {
            ...
            if (!des->isMutable) {
              newTensor = WrapExecution::copyConstCache(
                  t, curBackend, mCacheConstTensors, permitCodegen);
            } else if (des->usage == Tensor::InsideDescribe::CONSTANT) {
              newTensor = WrapExecution::copyConstCache(
                  t, curBackend, shapeFixConstCache, permitCodegen);
            }
              ...
              if (TensorUtils::getDescribe(t)->usage ==
                ...
                if (inputCacheIter != mInfo.first.inputTensorCopyCache.end()) {
                  ...
                  if (nullptr == std::get<0>(tensorCache) ||
                      WrapExecution::needWrap(std::get<0>(tensorCache),
                                              curBackend)) {
                  ...
                  if (std::get<2>(tensorCache)) {
                    auto allocRes = curBackend->onAcquireBuffer(
                        std::get<1>(tensorCache).get(), Backend::STATIC);
                    if (!allocRes) {
                      return OUT_OF_MEMORY;
                    }
            ...
            auto copyWrap = WrapExecution::makeCopyExecution(
                curBackend, mInfo.first.cache.second.get(), t, wrapCache, true);
      ...
      for (int v = 0; v < iter.workOutputs.size(); ++v) {
        ...
        if (WrapExecution::needWrap(t, curBackend)) {
          auto copyWrap = WrapExecution::makeCopyExecution(
              curBackend, mInfo.first.cache.second.get(), t, wrapCache, false);
...
ErrorCode Pipeline::allocMemory(bool firstMalloc, bool forbidReplace) {
  // MNN_PRINT("allocMemory mtype:%d, cpubackendType:%d, cpuBackend
  // runtime:%p\n", mBackend->type(), mBackupBackend->type(),
  // mBackupBackend->getRuntime());
  if (!firstMalloc) {
    // For session setNeedMalloc, if session's output is set as some input, It
    // may cause error Dup des to avoid it
    for (auto &info : mInfo.second) {
      auto &buffer = info.executeBuffer;
        ...
        for (auto t : info.workOutputs) {
          ...
          if (TensorUtils::getDescribeOrigin(t)->mContent->count() > 1) {
            TensorUtils::getDescribeOrigin(t)->mContent =
                new Tensor::InsideDescribe::NativeInsideDescribe;
            auto dstDes = TensorUtils::getDescribe(t);
            t->buffer().dim = dstDes->dims;
            ::memcpy(t->buffer().dim, des->dims,
                     MNN_MAX_TENSOR_DIM * sizeof(halide_dimension_t));
  ...
  {
    auto insertCode =
        _InsertCopy(mInfo, mCacheConstTensors, mShapeFixConstCache, mAllocInput,
                    forbidReplace);
    ...
    for (int cmdIndex = 0; cmdIndex < buffer.command.size(); ++cmdIndex) {
      ...
      if (nullptr != info.op->name()) {
        MNN_PRINT("%f, before Resize: %s - %d\n", memory,
                  info.op->name()->c_str(), cmdIndex);
      }
#endif

      // MNN_PRINT("before Resize: optype:%s, name:%s, input0:%p, output0:%p,
      // mAllocInput:%d\n", EnumNameOpType(iter.op->type()),
      // iter.info->name().c_str(), iter.inputs[0], iter.outputs[0],
      // mAllocInput); Alloc for Tensors
      auto curBackend = iter.execution->backend();
      if (mAllocInput) {
        for (auto t : iter.workInputs) {
...
void Pipeline::_copyInputs() {
  for (auto &iter : mInfo.first.inputTensorCopyCache) {
    ...
    auto curBackend =
        TensorUtils::getDescribe(std::get<0>(tensorCache))->getBackend();
    if (curBackend->type() == MNN_FORWARD_CPU) {
      TensorUtils::getDescribe(iter.first)
          ->getBackend()
          ->onCopyBuffer(iter.first, std::get<0>(tensorCache));
    } else {
...
// #define LOG_VERPOSE
      ...
      if (/* cmd.op->name() && cmd.op->name()->str() ==
             "/embed/embed_/Gather_output_0"*/
