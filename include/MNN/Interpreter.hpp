...
struct ScheduleConfig {
  ...
  struct Path {
    ...
    enum Mode {
       ...
       * The path will start from source op, then flow when encounter the sink
       * op. The sink op will not be compute in this path.
       */
      Op = 0,
  ...
  /** backup backend used to create execution when desinated backend do NOT
   * support any op */
...
typedef std::function<bool(const std::vector<Tensor *> &,
    ...
    TensorCallBack;
typedef std::function<bool(const std::vector<Tensor *> &, const OperatorInfo *)>
...
public:
  ...
  enum SessionMode {
    ...
    /** The input tensor is alloced by session, input data after session
       resized*/
    ...
    /** Determine static memory whether recyle in resizeSession or just cache
       the memory */
    Session_Memory_Collect = 10, // Recycle static memory when session resize in
                                 // case memory explosion
  ...
  enum HintMode {
    ...
    MAX_TUNING_NUMBER = 0,
    // Strictly check model file or not, default 1. if set 0, will not check
    // model file valid/invalid
...
public:
  ...
  /**
   * @brief create session with schedule config. created session will be managed
   * in net.
   ...
   * @param config session schedule config, runtime runtimeInfo used by the
   * created session.
   * @return created session if success, NULL otherwise.
   */
  Session *createSession(const ScheduleConfig &config,
  ...
  /**
   * @brief create multi-path session with schedule configs. created session
   * will be managed in net.
   * @param configs session schedule configs.
   * @return created session if success, NULL otherwise.
   */
  ...
  /**
   * @brief create multi-path session with schedule configs and user-specified
   runtime. created session will be managed in net.
   * @param configs session schedule configs.
   * @return created session if success, NULL otherwise.
   */
  Session *createMultiPathSession(const std::vector<ScheduleConfig> &configs,
  ...
  /**
   * @brief call this function to get tensors ready. output tensor buffer (host
   * or deviceId) should be retrieved after resize of any input tensor.
   * @param session given session.
   */
  ...
  /**
   * @brief call this function if don't need resize or create session any more,
   * it will save a few memory that equal to the size of model buffer
   */
  void releaseModel();
   ...
   * If model is not loaded or model no version info, return "version info not
   * found".
   */
   ...
   * @brief run session.
   * @param session   given session.
   * @param before    callback before each op. return true to run the op; return
   * false to skip the op.
   * @param after     callback after each op. return true to continue running;
   * return false to interrupt the session.
  ...
  enum SessionInfoCode {
    ...
    /** Backends in session in M, int*, length >= 1 + number of configs when
       create session */
    BACKENDS = 2,
  ...
  getSessionOutputAll(const Session *session) const;
  /**
   ...
   */
  const std::map<std::string, Tensor *> &
  getSessionInputAll(const Session *session) const;
...
public:
   ...
   * @param width / W
   */
  void resizeTensor(Tensor *tensor, int batch, int channel, int height,
                    int width);
...
private:
  ...
  void logForRunSession(const Session *session, float time,
